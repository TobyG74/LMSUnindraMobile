...
class PresensiMahasiswaScreen extends StatefulWidget {
  const PresensiMahasiswaScreen({super.key});

  @override
  State<PresensiMahasiswaScreen> createState() =>
      _PresensiMahasiswaScreenState();
}

class _PresensiMahasiswaScreenState extends State<PresensiMahasiswaScreen> {
    ...
    for (var row in rows) {
      try {
        ...
        if (pertemuanMatch != null) {
          pertemuanTerlaksana =
              int.tryParse(pertemuanMatch.group(1) ?? '0') ?? 0;
          totalPertemuan = int.tryParse(pertemuanMatch.group(2) ?? '16') ?? 16;
          if (totalPertemuan > 0) {
            persentasePertemuan =
                ((pertemuanTerlaksana / totalPertemuan) * 100).round();
          }
        }
